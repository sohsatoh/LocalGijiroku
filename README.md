...
- Streams audio through **WhisperKit** for real-time, multilingual transcription (Japanese and English tuned). The live transcript ships with the familiar per-segment rows layout (one boxed row per Whisper segment, italic + dimmed for the unconfirmed rolling tail), and Settings → General offers an opt-in Notion-style speaker-turn layout that collapses consecutive speech into prose blocks with the tail flowing inline. A single font size slider (10–22 pt) scales the transcript, summary, and tasks panes together. Confirmed segments are what get saved and shown to the LLM.
