...
- **WhisperKit** でリアルタイムに多言語文字起こし (日本語・英語を重点的にチューニング)。ライブ字幕は Notion AI 風の話者ターンブロック方式: 同一話者の連続発話は 1 ブロックに集約され、確定済みプロセを流し込みつつ、最新ブロック末尾に rolling tail を斜体・薄色で連結表示します。保存・LLM への入力は確定済みのみ。
