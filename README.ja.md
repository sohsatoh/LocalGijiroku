...
- 録音中だけメニューバーに現れる**録音コントロール**。メインウィンドウを前面に出さずに一時停止・再開・停止できるので、議事録中もスライドやノートに集中したまま操作可能。
...
| バックエンド | セットアップ | 備考 |
| --- | --- | --- |
| **MLX** (デフォルト) | 不要 — 初回利用時にモデルが自動ダウンロード。 | アプリプロセス内で完結。`mlx-community/` 配下から Qwen3 1.7B 〜 Qwen2.5 14B、Gemma 4 12B・Ministral 3 まで厳選収録。 |
