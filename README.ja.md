...
- **WhisperKit** でリアルタイムに多言語文字起こし (日本語・英語を重点的にチューニング)。デフォルトは「行ごと」レイアウトで、Whisper の各セグメントを 1 つの枠付き行として表示し、未確定の rolling tail は斜体・薄色でハイライトされます。設定 → 一般から Notion 風の話者ターン (連続発話を 1 ブロックにまとめ、tail をインライン表示) に切り替え可能。文字サイズは 10〜22 pt で調整できます。保存・LLM への入力は確定済みのみ。
