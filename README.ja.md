...
- **WhisperKit** でリアルタイムに多言語文字起こし (日本語・英語を重点的にチューニング)。ライブ字幕では「確定済み」テキストの末尾に未確定の rolling tail を斜体・薄色で表示し、Whisper が次の推論で書き換える可能性のある区間を視認できます。保存・LLM への入力は確定済みのみ。
...
```bash
swift test                                       # ~105 個の hermetic ユニットテスト
